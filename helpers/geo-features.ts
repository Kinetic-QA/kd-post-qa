...
export interface GeoFeatureConfig {
  ...
  slotsLinkText?: string; // optional — the footer/nav link's exact visible text for the Slots category. Defaults to 'Slots' when omitted. footer-navigation.spec.ts does an exact (anchored) text match, so a GEO whose real link reads "Online Slots" instead of "Slots" was silently recorded as "link not present for this GEO" even though it's right there — set this whenever a GEO's real label differs. Confirmed live: SNG ROW's footer/menu category is genuinely "Online Slots" (see its hasGameCategoryNav note).
...
export const GEO_FEATURES: Record<string, Record<string, GeoFeatureConfig>> = {
  SC: {
    ...
    ES:  { locale: 'es', uiLocalized: true,  hasBlog: true,  blogPath: 'blog/', hasPromotionsPage: true,  promotionsPath: 'promociones/',        featuresPath: 'funciones/',       mobileAppPath: 'app-casino-movil/', bingoCardGeneratorPath: 'generador-cartones-bingo/',    currencySymbol: '€', contactEmail: 'soporte@slingocasino.es', socialMedia: { twitter: 'slingoespana', facebook: 'slingospain', instagram: 'slingoespana' }, hasSocialMedia: true, searchTerm: 'Casino', searchResultHrefSubstrings: ['/slots/casino', '/casino/other/casino'], hasGameFilterCarousel: true, hasFeedbackForm: true, hasGameCategoryNav: true, hasLoginRegistration: true, hasAccountModal: true, hasPaymentMethodsPage: true, hasBlogDesktopSearch: true, hasBlogSearch: true, paymentMethodsPath: 'metodos-pago/' }, // confirmed live 2026-09-09: footer-navigation.spec.ts's Payment Options step landed on /metodos-pago/, not the guessed common 'payment-methods/' default (same real slug as GC ES) — field was missing entirely, same class of missing-localized-path gap already fixed elsewhere in this file
  ...
  SNG: {
    ...
    UK: {
      ...
      slotsLinkText: 'Online Slots', // confirmed live via curl 2026-08-24: footer's Slots link text is "Online Slots", NOT the plain "Slots" the game-category-nav's own SIDE MENU uses — different nav elements, genuinely different visible text, this was making footer-navigation.spec.ts falsely record it as absent for this GEO
    ...
    IE: {
      ...
      slotsLinkText: 'Online Slots', // confirmed live via curl 2026-08-24: footer's Slots link text is "Online Slots", same as UK/CA/ON/ROW — different from the side-menu's plain "Slots" text
      hasLoginRegistration: true,
    ...
    CA: {
      ...
      slotsLinkText: 'Online Slots', // confirmed live via curl 2026-08-24: footer's Slots link text is "Online Slots", same as UK/IE/ON/ROW
      hasLoginRegistration: true,
      hasAccountModal: true,
      hasPaymentMethodsPage: true, // confirmed 200
    ...
    'FR-CA': {
      ...
      slotsLinkText: 'Machines à sous', // confirmed live via curl 2026-08-24: footer's Slots link text is genuinely French, not "Slots" — footer-navigation.spec.ts's other hardcoded English category labels (Slingo/Bingo/Casino) likely have the same false-skip gap here, not yet independently checked
    ...
    ON: {
      ...
      slotsLinkText: 'Online Slots', // confirmed live via curl 2026-08-24: footer's Slots link text is "Online Slots", same as UK/IE/CA/ROW
    ...
    ROW: {
      ...
      slotsLinkText: 'Online Slots', // confirmed live — footer/menu category reads "Online Slots" here, not the hardcoded "Slots" default, which made footer-navigation.spec.ts's Slots step falsely record it as absent for this GEO
