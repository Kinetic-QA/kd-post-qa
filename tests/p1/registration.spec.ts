...
test.describe('Registration Flow', () => {
  ...
  test('REG-01: Complete new player registration flow', async ({ page }) => {
    ...
    // helpers below. SE not yet confirmed at all.
    ...
    // pattern as ROW (not SNG AB/CA's explicit-dropdown-selection case) —
    ...
    const isPcCaFormat = (process.env.TEST_BRAND ?? 'SC').toUpperCase() === 'PC'
      && test.info().project.name.replace(/-mobile$/, '') === 'CA';
    // Lord Ping (LP) CA — onboarding started 2026-07-29. Same auto-detect-
    // from-real-IP pattern as MC/PC's CA markets: country code needs no
    // explicit dropdown selection, just a real Canadian NANP-format mobile
    // number instead of the default UK-shaped one (confirmed live: the
    // plain fallback generator kept producing UK-style numbers that a real
    // Canadian site rejects on every retry, regardless of how many times
    // Step 0 retried).
    const isLpCaFormat = (process.env.TEST_BRAND ?? 'SC').toUpperCase() === 'LP'
      && test.info().project.name.replace(/-mobile$/, '') === 'CA';
    ...
    const isPslUkFormat = (process.env.TEST_BRAND ?? 'SC').toUpperCase() === 'PSL'
      && test.info().project.name.replace(/-mobile$/, '') === 'UK';
    // Prime Scratch Cards (PSC) UK — onboarded 2026-08-03, brand-new brand.
    // Confirmed live this generic-default fallback path was WRONG for this
    // brand: a real run failed retrying "consent checkbox gdprBingo did not
    // get checked" because PSC (like PSL) genuinely has no Bingo vertical
    // at all (taxonomy is Scratch Cards/Slots/Casino, confirmed via
    // `.main-tabs` DOM dump) — same no-Bingo 3-checkbox set as PSL UK.
    const isPscUkFormat = (process.env.TEST_BRAND ?? 'SC').toUpperCase() === 'PSC'
      && test.info().project.name.replace(/-mobile$/, '') === 'UK';
    // Lucky Me Slots (LMS) UK — onboarded 2026-08-03, brand-new brand.
    // Confirmed live via a real run: without this branch, Step 3 failed for
    // real ("consent checkbox gdprBingo did not get checked") because LMS's
    // taxonomy (Online Slots/Progressive Jackpots/Table Games/Live Casino/
    // Game List, confirmed via its sidebar's own DOM) has no Bingo vertical
    ...
    const isPslCaFormat = (process.env.TEST_BRAND ?? 'SC').toUpperCase() === 'PSL'
      && test.info().project.name.replace(/-mobile$/, '') === 'CA';
    // Prime Scratch Cards (PSC) CA — onboarded 2026-08-03. Confirmed live
    // via a real run: without this branch, Step 0 fell through to the
    // plain UK-shaped default and failed for real ("mobile not accepted
    // after 10 attempts", full 180s timeout) — same exact bug class as
    // every other brand's CA market. Same underlying platform as PSL CA
    // (confirmed live: real login redirects to
    ...
    if (isSpanishFormat && isMobile) {
      ...
      await runStep('Step 0: Mobile + Date of Birth → Continue', async () => {
        ...
        await ((isAlbertaFormat || isCanadianMobileFormat)
          ...
          : (isSgComFormat || isLpComFormat || isPslComFormat || isPscComFormat || isLmsComFormat)
      ...
      await runStep('Step 2 of 5: Gender + Email → Continue', async () => {
        ...
        await ((isCanadianMobileFormat || isPcCaFormat || isPcComFormat || isMcComFormat || isMcCaFormat || isMcFrCaFormat || isLpCaFormat || isPslCaFormat || isPscCaFormat || isLmsCaFormat || isSgCaFormat || isSgComFormat || isLpComFormat || isPslComFormat || isPscComFormat || isLmsComFormat)
      ...
      await runStep('Step 3 of 5: Address → Continue', async () => {
        ...
        await (usesAbAddressShape
          ...
          : (isCanadianMobileFormat || isPcCaFormat || isPcComFormat || isMcComFormat || isMcCaFormat || isMcFrCaFormat || isLpCaFormat || isPslCaFormat || isPscCaFormat || isLmsCaFormat || isSgCaFormat || isSgComFormat || isLpComFormat || isPslComFormat || isPscComFormat || isLmsComFormat)
      ...
      await runStep('Step 5 of 5: Deposit limit + consents', async () => {
        ...
        await (usesAbAddressShape
          ...
          : (isPcUkFormat || isPcCaFormat || isPcComFormat || isLpUkFormat || isMcComFormat || isMcCaFormat || isLpCaFormat || isI36NoBingoFormat || isPslUkFormat || isPslCaFormat || isPscUkFormat || isPscCaFormat || isLmsUkFormat || isLmsCaFormat || isSgUkFormat || isSgCaFormat || isSgComFormat || isLpComFormat || isPslComFormat || isPscComFormat || isLmsComFormat)
      ...
      await runStep('Step 1: Name + Email + Gender → Continue', async () => {
        ...
        await fillStep1(page, scope, data, ((isCanadianMobileFormat && !isOntarioFormat) || isMcComFormat || isMcCaFormat || isMcFrCaFormat || isPcCaFormat || isPcComFormat || isLpCaFormat || isPslCaFormat || isPscCaFormat || isLmsCaFormat || isSgCaFormat || isSgComFormat || isLpComFormat || isPslComFormat || isPscComFormat || isLmsComFormat)
      ...
      await runStep('Step 2: Address → Continue', async () => {
        ...
        await (usesAbAddressShape ? fillStep2AB(page, scope, data)
          ...
          : (isMcComFormat || isMcCaFormat || isPcCaFormat || isPcComFormat || isLpCaFormat || isPslCaFormat || isPscCaFormat || isLmsCaFormat || isSgCaFormat || isSgComFormat || isLpComFormat || isPslComFormat || isPscComFormat || isLmsComFormat) ? fillComAddress(page, scope, data)
      ...
      await runStep('Step 3: Username + Password + Checkboxes', async () => {
        ...
        await (usesAbAddressShape
          ...
          : (isMcComFormat || isMcCaFormat || isPcUkFormat || isPcCaFormat || isPcComFormat || isLpUkFormat || isLpCaFormat || isI36NoBingoFormat || isPslUkFormat || isPslCaFormat || isPscUkFormat || isPscCaFormat || isLmsUkFormat || isLmsCaFormat || isSgUkFormat || isSgCaFormat || isSgComFormat || isLpComFormat || isPslComFormat || isPscComFormat || isLmsComFormat)
          ? fillStep3(page, scope, data, ['over_18', 'gdpr', 'terms_accept'])
...
async function fillStep0WithRetry(
  ...
  for (let attempt = 1; attempt <= MAX_MOBILE_RETRIES; attempt++) {
    ...
    await dismissCampaignPopup(page).catch(() => {});

    // Confirmed live on LP CA 2026-07-29: the "What's your first name?"
    // label sits directly above the textbox as plain visual text, not
    // programmatically associated via <label for>/aria-labelledby — the
    // textbox's computed accessible name is empty, so a role-based name
    // match never finds it even though the field is genuinely visible and
    // the form genuinely did advance. getByText matches the visible label
    // text itself regardless of ARIA association, so it works whether or
    // not a brand's markup links the two properly.
    // IMPORTANT: isVisible({timeout}) does NOT poll/retry for the element to
    // appear — unlike expect().toBeVisible() or waitFor(), it resolves
    // almost immediately if the element isn't already there, so bumping the
    // timeout number does nothing. waitFor({state:'visible'}) is the one
    // that actually polls, which is what this needs after a page transition
    // whose exact render time can vary run to run.
    const onStep1 = await scope.getByRole('textbox', { name: /first name|pr[ée]nom/i })
      ...
      || await scope.getByText(/first name|pr[ée]nom/i).first().waitFor({ state: 'visible', timeout: 8_000 }).then(() => true).catch(() => false);
 ...
 * auto-detect pattern as SC's ROW), so forcing a selection the way
